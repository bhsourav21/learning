...
vectors_query = np.array(response_query.data[0].embedding)
vectors_texts = np.array([item.embedding for item in response_texts.data])

print(f"\n[OpenAI text-embedding-3-small] — Top 3 results")
print(f"Query: \"{query}\"\n")
print(f"{'#':<5} {'Similarity':<12} Text")
print("-" * 100)

scores = [(cosine_similarity(vectors_query, vectors_texts[i]), text) for i, text in enumerate(texts)]
