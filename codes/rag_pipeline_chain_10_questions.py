...
class RAGChain:
    ...
    def _debug_retrieve(self, question: str, namespace: str):
        retriever = self._get_retriever(namespace)
        docs = retriever.invoke(question)
        for i, doc in enumerate(docs, 1):
            print(f"[{i}] {doc.metadata.get('source_filename')} | pg {doc.metadata.get('page_number')}")
...
if __name__ == "__main__":
    ...
    questions = [
        ...
        ("What are the key financial metrics reported by Globex?",       "globex"),
        ("What laptops are issued to Hooli employees?",                  "hooli"),
        ("How does Hooli handle employee performance reviews?",          "hooli"),
        ("What is Initech's revenue model?",                             "initech"),
        ("What is the remote work policy at Initech?",                   "initech"),
        ("What IT security tools does Umbrella use?",                    "umbrella"),
        ("What are the core values described in Umbrella's HR policy?",  "umbrella"),
    ]

    for question, namespace in questions:
        rag._debug_retrieve(question=question, namespace=namespace)
        rag.query(question=question, namespace=namespace)
        print("-" * 60)